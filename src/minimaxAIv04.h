// Copyright 2046 ghooo (https://github.com/ghooo)
...
#ifndef MINIMAXAIV04_H
#define MINIMAXAIV04_H

#include "botAI.h"

class MinimaxAIv04: public BotAI {
public:
	MinimaxAIv04(Field* field) : BotAI(field) {
		dirc[0] = 0, dirc[1] = 1, dirc[2] = 1, dirc[3] = 1; 
		dirr[0] = 1, dirr[1] = 1, dirr[2] = 0, dirr[3] = -1;
		eta = 0.5;
		// 	double weightPlayer[2], weightOrder[6], weightNeighbors[4];
		//(6-1-order)*neighbors*
		for(int order = 0; order < 6; order++) {
			int t = 6-1-order;
			// weightOrder[order] = t*t*t;
			weightOrder[order] = t;
		}

		weightNeighbors[2] = 2;
		weightNeighbors[3] = weightNeighbors[2]*3;
		weightNeighbors[0] = weightNeighbors[1] = 0.0;
		// weightPlayer[0] = 1.2;
		weightPlayer[0] = 1;
		weightPlayer[1] = 1;
	}
...
private:
	int evaluate(Field* field) {
		...
		for(int order = 0; order < 6; order++) {
			for(int neighbors = 0; neighbors < 4; neighbors++) {
				ret += getWeight(0,order,neighbors,count[0][order][neighbors]);
				ret -= getWeight(1,order,neighbors,count[1][order][neighbors]);
			}
		}
	...
	int getWeight(int playerIdx, int order, int neighbors, int count) {
		if(getBotId() == 2) {
			return count*neighbors*neighbors;
		} else {
			return count*neighbors*neighbors*(5-order);
		}
		return count*weightPlayer[playerIdx]*weightOrder[order]*weightNeighbors[neighbors];
	}
